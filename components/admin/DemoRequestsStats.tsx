...
interface DemoRequestsStatsData {
  total: number;
  pending: number;
  scheduled: number;
  completed: number;
...
export default function DemoRequestsStats() {
  const [stats, setStats] = useState<DemoRequestsStatsData | null>(null);
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch('/api/admin/demo-requests/stats');
      if (response.ok) {
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: '📊',
      color: 'bg-pink-900/50 text-pink-200',
